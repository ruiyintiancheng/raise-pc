    ...
    <el-dialog
      ...
      <el-form v-if="formVisible" ref="ruleForm" :model="ruleForm" :rules="rules">
        ...
        <el-form-item style="margin-top:0px;">
            ...
            <el-input :rows="4" onkeyup="value=value.replace(/[^\d]/g,'')" v-model="ruleForm.duration" placeholder="请输入数字"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      status: 0, // 0添加 1修改
      message: [
        { title: '添加', url: '/objective/rwDaily/add' },
        { title: '修改', url: '/objective/rwDaily/update' },
        { title: '查看' }
      ],
      subbtn: false,
  ...
  methods: {
    ...
    openDialog(row) {
      ...
      if (row) { // 修改
        ...
        this.params = {
          ...
          dailyType: row.dailyType
        }
        this.ruleForm = {
          dailyMatter: row.dailyMatter,
    ...
    async trip() {
      ...
      // 添加的时候先查询日程是否冲突
      // const trip = {
      //   dailyId: this.params.dailyId || '',
      //   dailyDate: this.params.dailyDate,
