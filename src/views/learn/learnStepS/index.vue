       ...
       <div class="learn-container inside-container" v-if="relation[currentIndex]">
          <div class="learn-head">
            <h3 class="learn-title">
              <span class="learn-title-f" @click="$router.push({name:'learn'})">我的课程</span> <span class="sp-line">/</span><span @click="$router.push({name:'firstStep'})" class="learn-title-f">First Step</span> <span class="sp-line">/</span><span class="learn-title-s">{{relation[currentIndex]['name']}}</span>
              <span class="learn-title-f strendthen" v-if="hasStrendthen || meetingOver" ><svg-icon style="margin-right:5px;" icon-class="gift" /><router-link :to="{name:'strengthen'}">First Step辅导升级包 </router-link></span>
            </h3>
          ...
          <template v-if="pageExist">
            ...
            <assessment ref="detailStep" v-if="assessments.some(item => item === relation[currentIndex]['step'])" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo"></assessment>
            <challenge ref="detailStep" v-if="challenges.some(item => item === relation[currentIndex]['step'])" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo" :overSecondWeek="relation.findIndex(it => it.step === flowNodeNo) < relation.findIndex(it => it.step === 14)"></challenge>
            ...
            <next-process ref="detailStep" v-if="nextProcesses.some(item => item === relation[currentIndex]['step'])" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo"></next-process>
            <meeting ref="detailStep" v-if="meetings.some(item => item === relation[currentIndex]['step'])" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo" :hs="hasStrendthen" @changeGoOnStatus="changeGoOnStatus" @changeStrendthen="changeStrendthen"></meeting>
            ...
            <tip ref="detailStep" v-if="tips.some(item => item === relation[currentIndex]['step'])" :currentTab="currentTab" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo"></tip>
            <graduation ref="detailStep" v-if="graduations.some(item => item === relation[currentIndex]['step'])" :currentTab="currentTab" :courseId="courseId" :flowNodeNo="relation[currentIndex]['step']" :isNewest="relation[currentIndex]['step'] === flowNodeNo" :graduationsName="graduationsName" @clearGraduationsName="graduationsName = ''"></graduation>
...
export default {
  ...
  watch: {
    currentIndex(to, from) {
      ...
      this.meetingTip = 0
      // if (firstCroseNos.includes(this.flowNodeNo) && firstCroseNos.includes(currentNo)) {
      if (firstCroseNos.includes(currentNo)) {
        this.getCourseTip(currentNo)
      }
      // if (secondCroseNos.includes(this.flowNodeNo) && secondCroseNos.includes(currentNo)) {
  ...
  computed: {
    ...
    // hasStrendthen() {
    //   const pointIndex = this.relation.findIndex(item => item.step === 12)
    //   const lastIndex = this.relation.findIndex(item => item.step === this.flowNodeNo)
  ...
  methods: {
    ...
    getHasStrendthen() {
      baseRequest('/user/rwProductUserNum/getBuyExtraFlag').then(response => {
        this.hasStrendthen = response.data.item.result
      })
    },
    /**
     * 点击继续
     ...
     * 子组件改变是否可以购买强化辅导
     */
    changeStrendthen(val) {
      this.meetingOver = val
      console.log(this.meetingOver)
    },
    // 获取小组提示
