<template>
    <div class="text-a-file">
        <div class="tf-head">
            <el-input style="width:300px;" :disabled="!noDisabled" v-model="imgItem.answer" placeholder="" maxlength="10"></el-input>
            <el-button style="margin-left:8px;" v-if="!noDisabled" @click="noDisabled = true" type="primary" size="small" :loading="textLoading">修改</el-button>
            <el-button style="margin-left:8px;" v-else type="success" size="small" @click="saveInput" :loading="textLoading">保存</el-button>
            ...
            <div class="img-box">
                ...
                <el-upload
                    ...
                    :auto-upload="true">
                    <i  class="el-icon-upload avatar-uploader-icon">{{uploadTip}}</i>
                </el-upload>
            </div>
        </div>
  ...
  methods: {
    ...
    saveInput() {
      ...
      if (this.imgItem.add) {
        ...
        params.id = this.imgItem.id
      }
      params.answer = this.imgItem.answer
      params.pid = this.pid
      params.days = this.days
      baseRequest('/course/rwReflectionDiary/operationCompletion', params).then(response => {
        this.$Message.success('操作成功')
