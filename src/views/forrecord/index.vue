         ...
         <el-row>
             ...
             <el-col :span="8">
                <el-form-item class="formitem">
                    <div>积分</div>
                    <el-form-item prop="changeCard">
                    <el-input v-model="form.changeCard" oninput="value=value.replace(/[^\d]/g,'')" style="width:180px" clearable></el-input>
                    </el-form-item>
                    <el-form-item>
                    <span>至</span>
                    </el-form-item>
                    <el-form-item prop="changeCards">
                    <el-input v-model="form.changeCards" oninput="value=value.replace(/[^\d]/g,'')" style="width:180px" clearable></el-input>
                    </el-form-item>
                </el-form-item>
             </el-col>
         </el-row>
...
<el-table class="table-main" :data="tableData"  style="width:100%;margin-top:0px;" v-loading="loading" border>
      ...
      <el-table-column prop="productName" label="兑换产品"  align="center"></el-table-column>
      <el-table-column prop="comment" label="产品属性"  align="center"></el-table-column>
      <el-table-column prop="give" label="是否赠送"  align="center"></el-table-column>
      <el-table-column prop="changeCards" label="扣除积分"  align="center"></el-table-column>
      <el-table-column prop="txnTime" label="兑换时间"  align="center"></el-table-column>
    </el-table>
        <!-- 全部列表分页 -->
...
export default {
  ...
  data() {
    var checkInput = (rule, value, callback) => {
      ...
      } else if (parseInt(this.form.changeCard) > parseInt(value)) {
        callback(new Error('请按规则输入区间值'))
      }
    }
    var checkInput1 = (rule, value, callback) => {
      if (this.form.changeCard % 5 !== 0) {
        callback(new Error('请按规则输入区间值'))
      }
    }
    return {
  ...
  methods: {
    exportHistory() {
      ...
      const params = {
        ...
        changeCard: this.form.changeCard,
        changeCards: this.form.changeCards
      }
      download('/user/rwCardsInfo/rwRedeemPointsExport', params, _ => {
    ...
    searchOption(page) {
      ...
      baseSearch('/user/rwCardsInfo/rwRedeemPointsList', data).then((response) => {
