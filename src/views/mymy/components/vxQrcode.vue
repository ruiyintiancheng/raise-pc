...
<!-- 上传图片 -->
    <div class="Qrcode">
        <div class="img-box" v-loading="imageLoading">
            ...
            <div v-else class="text-tip">请上传二维码</div>
            <!-- <span class="qrSpan" v-if="jxhImage?false:true">请上传二维码</span> -->
        </div>
        <div class="codeUplod">
          <el-upload
  ...
  methods: {
    ...
    getQrCode() {
      baseRequest('/user/rwUser/getPersonalUserInfo').then((response) => {
        // this.base64Code = response.data.item
        // this.jxhImage = ''
        this.jxhImage = response.data.item.coachQrCode
      })
    },
    getFile(file) {
