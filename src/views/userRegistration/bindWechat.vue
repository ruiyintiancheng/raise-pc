 ...
 * @Last Modified time: 2020-11-13 16:59:21
 * @Description:  绑定微信
 */
<template>
          ...
          <el-form  ref="form" :model="form" class="login-form">
            ...
            <template v-if="step === 2">
              ...
              <el-form-item >
                ...
                <el-form-item >
                <el-input v-model.trim="form.email" placeholder="请输入邮箱" clearable :class="{borderG:emailColor===1,borderR:emailColor===2}" @focus="focusColor('emailColor')" @blur="blurColor('emailColor')"></el-input>
              </el-form-item>
            </template>
...
export default {
  ...
  methods: {
    ...
    handleLogin() {
      if (this.step === 1) {
        ...
        encrypt(this.form.loginPasswd).then(password => {
          const params = { userName: this.form.userName, loginName: this.form.loginName, loginPasswd: password, email: this.form.email, status: this.status, unionId: this.$route.query.unionId }
          baseRequest('/user/userLogin/registerUserAndWeChat', params).then(response => {
            if (response.code === 2) {
    ...
    stepOne() {
      ...
      this.loading = true
      const params = { loginName: this.form.loginName, smsCode: this.form.smsCode, unionId: this.$route.query.unionId }
      // this.step = 2
      this.loading = false
      baseRequest('/user/userLogin/initWechatLogin', params).then(response => {
        if (response.code === 2) {
          const validateMsg = response.data
          if (validateMsg.hasOwnProperty('loginName')) {
            this.nameColor = 2
        ...
        if (status === '1') {
          ...
          this.$confirm('检测到您的账户已绑定了其它微信，如要绑定新的微信账号，原绑定微信号码将自动解绑，如果继续请点击确定。', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            baseRequest('/user/userLogin/updateWeChat', { loginName: this.form.loginName, unionId: this.$route.query.unionId }).then(response => {
              this.$store.dispatch('LoginByUsername', response.data.item.token).then(() => {
                this.$router.push('/')
              })
            })
